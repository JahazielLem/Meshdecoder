...
static int dissect_meshtastic(tvbuff_t* tvb,
  ...
  if (gcry_cipher_decrypt(cipher_hd, chiper_payload_block, payload_len, NULL,
                          0)) {
  ...
  tvbuff_t* tvb_decrypted = tvb_new_child_real_data(tvb, chiper_payload_block,
  ...
  // Bitfield
  proto_tree_add_item(subtree_decrypted, hf_mstic_bitfield, tvb_decrypted,
                      decrypted_offset, 1, ENC_NA);
  decrypted_offset += 1;

  // Portnum
  uint8_t portnum = tvb_get_uint8(tvb_decrypted, decrypted_offset);
  proto_tree_add_item(subtree_decrypted, hf_mstic_type_packet, tvb_decrypted,
                      decrypted_offset, 1, portnum);
  col_set_str(
  ...
  if (portnum == 1) {
    ...
    // Payload Len
    uint8_t text_app_msg_len = tvb_get_uint8(tvb_decrypted, decrypted_offset);
    proto_tree_add_item(subtree_decrypted, hf_mstic_textapp_message_len,
                        tvb_decrypted, decrypted_offset, 1, ENC_NA);
    decrypted_offset += 1;

    proto_tree_add_item(subtree_decrypted, hf_mstic_textapp_message,
                        tvb_decrypted, decrypted_offset, text_app_msg_len,
                        ENC_NA);
