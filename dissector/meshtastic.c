 ...
 */
 ...
 static int dissect_meshtastic(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data)
 {
   ...
   uint32_t senderID = tvb_get_ntohl(tvb, current_offset);
   ...
   uint32_t packetID = tvb_get_ntohl(tvb, current_offset);
   ...
   memcpy(iv + MESHTASTIC_ADDR_LEN, &senderID, sizeof(senderID));

   uint8_t* encrypted_payload = tvb_me
