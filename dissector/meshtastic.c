...
static int dissect_meshtastic(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data) {
   ...
   proto_tree_add_uint(ti_radio, hf_meshtastic_dst, tvb, current_offset, MESHTASTIC_ADDR_LEN, dst_addr);
   ...
   proto_tree_add_uint(ti_radio, hf_meshtastic_src, tvb, current_offset, MESHTASTIC_ADDR_LEN, src_addr);
   ...
   proto_tree_add_uint(ti_radio, hf_meshtastic_pktid, tvb, current_offset, MESHTASTIC_PKTID_LEN, packetid);
   current_offset += MESHTASTIC_PKTID_LEN;
 
   // Flags
   uint8_t flags_value = tvb_get_uint8(tvb, current_offset);
   proto_item *ti_flags = proto_tree_add_uint_format(ti_radio, hf_meshtastic_flags, tvb, current_offset, 1, flags_value, "Flags: 0x%02x", flags_value);
   ...
   */
   ...
   memcpy(iv + MESHTASTIC_ADDR_LEN + MESHTASTIC_ADDR_LEN, &senderID, sizeof(senderID));

   uint8_t* chiper_payload_block = tvb_memdup(pinfo->pool, tvb, current_offset, payload_len);
   ...
   if (gcry_cipher_decrypt(cipher_hd, chiper_payload_block, payload_len, NULL, 0)) {
 ...
 void proto_register_meshtastic(void)
 {
   ...
   static hf_register_info hf[] = {
       {&hf_meshtastic_dst, {"Destination", "meshtastic.dst", FT_UINT32, BASE_HEX, NULL, 0x0, "Destination Address", HFILL}},
       {&hf_meshtastic_src, {"Source", "meshtastic.src", FT_UINT32, BASE_HEX, NULL, 0x0, "Source Address", HFILL}},
       {&hf_meshtastic_pktid, {"Packet ID", "meshtastic.pktid", FT_UINT32, BASE_HEX, NULL, 0x0, NULL, HFILL}},
       {&hf_meshtastic_flags, {"Flags", "meshtastic.flags", FT_UINT8, BASE_HEX, NULL, 0x0, NULL, HFILL}},
       {&hf_meshtastic_flags_hop_limit, {"Hop Limit", "meshtastic.hop_limit", FT_UINT8, BASE_DEC, NULL, 0x07, NULL, HFILL}},
   ...
   // Register dissectors
   handle_meshtastic = register_dissector("meshtastic", dissect_meshtastic, proto_meshtastic);
