...
class Decrypter:
  ...
  def decode_protobuf(self, packet_data, sourceID, destID):
    ...
    try:
      ...
      try:
        ...
        pos.ParseFromString(packet_data)
        return pos
      except:
        pass
      try:
        pos = mesh_pb2.FromRadio()
        pos.ParseFromString(packet_data)
        return pos
      except:
        pass
    ...
    match data.portnum:
      ...
      case 1: # Text Message
        ...
        data = f"Text Message: {data.request_id} {data.reply_id} {str(text_payload)}\nBitfield: {data.bitfield}\nEmoji: {data.emoji}\nwant_response: {data.want_response}\n{data.payload}"
      case 3 : # POSITION_APP
        try:
          print(data.portnum)
          pos = mesh_pb2.Position()
  ...
  def show_details(self, mesh_packet):
    ...
    print(len(dec_data) - 4, dec_data)
    print(dec_data[0], "-> Bitfield")
    print(dec_data[1], "-> portnum")
    print(dec_data[2])
    print(dec_data[3])
    print(len(dec_data[4:8]), dec_data[4:8])
    print(dec_data[-3])
    print(dec_data[-2])
    print(dec_data[-1])
...
if __name__ == "__main__":
    ...
    lest_message = [
      ...
      # Position
      # b'\xff\xff\xff\xffP\xcd]\xa4{\x92{scw\x00\x00(\xc9\xea\xd6O\xee\xbe\xdf\xcc\xfc\x8c'.hex(),
      # b'\xff\xff\xff\xffP\xcd]\xa4{\x92{sbw\x00P(\xc9\xea\xd6O\xee\xbe\xdf\xcc\xfc\x8c'.hex(),
      # b'\xff\xff\xff\xffP\xcd]\xa4\x1a\xe3\x03\x8acw\x00\x00N\x8b\xae~\xef!x\x87\xc2\xe5\x1fm`\xf9J\xc03\xe1\x85.#\xbaa\x84\xce\x1a\x81\xae\x1fl\x1f\x1e\xf3\xb8\xd7\xc5]\x91\xf7\xfd\xcb(\xf2\x01\xf25S\xfdN\xf3\xe5\x10\x85Z\xa3<H\xdcD\xe7\xd0X'.hex(),
      # b'\xff\xff\xff\xffP\xcd]\xa4\xdd\xef\xd9a\x00w\x00\x00\xbafR\x05\x0b\xea\x06U\xc4\xa4\xba\xe9\xf9\\\x99P#\x16v\xea\xe3\x93\xden\xd71C\x87\xdb|P\x93'.hex(),
      # b'\xff\xff\xff\xffP\xcd]\xa4\x1a\xe3\x03\x8abw\x00PN\x8b\xae~\xef!x\x87\xc2\xe5\x1fm`\xf9J\xc03\xe1\x85.#\xbaa\x84\xce\x1a\x81\xae\x1fl\x1f\x1e\xf3\xb8\xd7\xc5]\x91\xf7\xfd\xcb(\xf2\x01\xf25S\xfdN\xf3\xe5\x10\x85Z\xa3<H\xdcD\xe7\xd0X'.hex(),
      b'\xff\xff\xff\xffP\xcd]\xa4\x9d\xb4\x87|cw\x00\x001\xfa\xff\xae\xb5p\xca\xb4\xbb\xbe!'.hex(),
      # b'\xff\xff\xff\xffP\xcd]\xa4\x9d\xb4\x87|bw\x00P1\xfa\xff\xae\xb5p\xca\xb4\xbb\xbe!'.hex(),
      b'\xff\xff\xff\xffP\xcd]\xa4\x0f\x9eS\x84bw\x00P\x84\x90l\xf5>N\tf\xbb\x1e7\x0f#T\x91F;\xa20t\x0eB?q'.hex(),
      b'\xff\xff\xff\xffP\xcd]\xa4\xa3\x9b\xec\xbabw\x00P\xb9|\x06\xaa\xcf\x84T'.hex(),
      b'\xff\xff\xff\xffP\xcd]\xa4jzjNbw\x00PD\x85~&\x9a\xe4\xb2\xe0\x82|'.hex(),
